...
**Tech stack:** Node.js, Express, SQLite via better-sqlite3, Alpha Vantage API, Finnhub API, Gemini API

---

## How to run

1. Copy `.env.example` to `.env` and fill in `ALPHA_VANTAGE_API_KEY`, `FINNHUB_API_KEY`, `GEMINI_API_KEY` (optional: `GEMINI_MODEL`).
2. `npm install`
3. `node server.js`
4. Open [http://localhost:3000](http://localhost:3000)

Optional (dev only): `node scripts/seedTestCache.js` loads fake board/cache rows so the UI works without burning Alpha Vantage quota. Remove that seed data before relying on live market pulls.
...
- **Lookup:** Search any ticker; results are cached in SQLite for 24 hours (`stock_cache` + `ai_summaries`) so repeat views do not re-hit paid/rate-limited APIs.
...
- **Watchlist:** Star tickers from the UI; stored locally in SQLite.
