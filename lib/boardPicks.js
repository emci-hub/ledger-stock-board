...
async function listBoardDisplayPicks() {
  return dbAll(
    `SELECT ticker, status, added_at, sector, archived_at, source,
            name, price, percent_change, exchange, discovered_at, last_seen_at,
            miss_streak, flags_json, tracked_since, board_section, tier,
            primary_ticker, primary_exchange, long_term_verdict, long_term_detail_json
     FROM board_picks
...
/**
 * Upsert a candidate discovered via a BlackRock/iShares factor ETF (USMV/
 * QUAL/MTUM). Unlike upsertCandidateFromUniverse, this ALSO updates
 * factor_tag/factor_rank on already-live/archived tickers — factor rank
 * (real published weight) is meant to stay current, not just apply once at
 * discovery time, since it directly drives display ordering.
 */
async function upsertCandidateFromFactor(entry) {
  const symbol = String(entry?.ticker || entry?.symbol || "")
    .trim()
    .toUpperCase();
  if (!symbol) return { action: "skipped_invalid" };
  ...
  const now = new Date().toISOString();
  const name = entry.name || null;
  const factorTag = entry.factorTag || null;
  const factorRank = Number.isFinite(Number(entry.factorRank))
    ? Number(entry.factorRank)
    : null;
  const existing = await getPick(symbol);
    ...
    await dbRun(
      `UPDATE board_picks
       SET name = COALESCE(?, name),
           ...
           factor_rank = ?
       WHERE ticker = ?`,
      [name, now, factorTag, factorRank, symbol]
    );
    return { action: "refreshed_live", ticker: symbol };
  }
  ...
  if (existing && existing.status === "archived") {
    ...
    return { action: "refreshed_archived", ticker: symbol };
  }
  ...
  await dbRun(
    `INSERT INTO board_picks
      (ticker, status, added_at, sector, archived_at, source,
       name, price, percent_change, exchange, discovered_at, last_seen_at,
       miss_streak, flags_json, factor_tag, factor_rank)
     VALUES (?, 'candidate', ?, NULL, NULL, 'factor_etf',
             ?, NULL, NULL, NULL, ?, ?, 0, ?, ?, ?)
     ON CONFLICT(ticker) DO UPDATE SET
       status = 'candidate',
       name = COALESCE(excluded.name, board_picks.name),
       ...
       flags_json = excluded.flags_json,
       source = COALESCE(board_picks.source, excluded.source),
       factor_tag = excluded.factor_tag,
...
async function removeCandidateIfPresent(ticker) {
  ...
  return { removed: Boolean(result?.rowsAffected) };
}
...
function rankCandidatesForPromotion(rows) {
  return (rows || [])
    .map((row) => {
      const flags = parseFlags(row.flags_json);
      const rawStrength = Math.abs(Number(row.percent_change) || 0);
      // Factor-sourced candidates (USMV/QUAL/MTUM) often have no today's
      // %change yet (not fetched), which would sink them to the bottom of
      // a pure momentum-strength sort despite having real evidence behind
      // them (a professional factor ranking). Give them a synthetic boost
      // scaled by their REAL published rank — rank 1 (most significant)
      // gets the biggest boost, tapering off. This lets them compete fairly
      // in the SAME single waterfall instead of needing a third parallel
      // seat-carve system (avoids repeating the complexity/bug surface we
      // already have with the universe floor).
      const factorRank = Number.isFinite(Number(row.factor_rank))
        ? Number(row.factor_rank)
        : null;
      const factorBoost =
        row.factor_tag && factorRank != null
          ? Math.max(0, FACTOR_BOOST_CAP - factorRank * FACTOR_BOOST_TAPER)
          : 0;
      const strength = Math.max(rawStrength, factorBoost);
 ...
 *
 * Ordering matters here: refresh jobs share one daily API quota across every
 * live ticker and stop once it runs out. listLiveBoardPicks() sorts newest
 * first, which meant the long-term section (a handful of names, usually the
 * oldest-added — e.g. AAPL/MSFT/JPM) was always last in line and got
 * starved by the much larger short/penny pool every single day. Long-term
 * names go first here (oldest-added within that group first, since those
 * are the most neglected), then everything else keeps its existing order.
 * Fixed 2026-08-21 after confirming AAPL/MSFT/JPM/etc. were 5-8 days stale
 * with missing analyst targets while short/penny names refreshed daily.
 */
async function getActiveBoardTickers() {
  ...
  for (const p of picks) {
    if (String(p.board_section) === "long") long.push(p);
    else rest.push(p);
  }
  long.sort((a, b) => new Date(a.added_at) - new Date(b.added_at));
  return [...long, ...rest].map((p) => String(p.ticker).toUpperCase());
}
 ...
 *
 * Long-section tickers are excluded from this pool entirely (fixed
 * 2026-08-23) — a calm, stable Long stock is SUPPOSED to have low momentum,
 * so comparing it against Short/Penny movers on raw changePercent always
 * made it look "weakest" and evicted it first, purely for doing exactly
 * what it's supposed to do (this is what archived AAPL via "board_cap" on
 * 2026-08-23). Long already has its own correctly-scoped ceiling via
 * ensureSectionCapacity()/findWeakestInSection() (see below) — it doesn't
 * need protecting from the overall board cap too.
 */
async function findWeakestLivePick(excludeTickers = []) {
  const { BOARD_SECTIONS } = require("./rankingStability");
  const exclude = new Set(
    (excludeTickers || []).map((t) => String(t).toUpperCase())
  );
  const picks = (await listLiveBoardPicks()).filter(
    (p) =>
      !exclude.has(String(p.ticker).toUpperCase()) &&
      p.board_section !== BOARD_SECTIONS.LONG
  );
  ...
  for (const pick of picks) {
    ...
    // FIX (2026-08-17): null (no fresh momentum data) must be treated as
    // the MOST evictable state, never protected against a real number.
    // The old branches (`else if (a == null) weakest = candidate` /
    // `else if (b == null) // keep weakest`) did the opposite — ANY real
    // momentum value, even a genuine +45% mover, was unconditionally
    // treated as "weaker" than an unknown, which could get a strong
    // performer wrongly evicted just from unlucky iteration order against
    // a data-gap ticker. Confirmed via direct trace before fixing.
    if (a == null && b == null) {
      if (String(candidate.added_at) < String(weakest.added_at)) weakest = candidate;
    } else if (a == null) {
      // weakest already has no data — it stays the most evictable
      // regardless of candidate's real value; do nothing.
    } else if (b == null) {
      // candidate has no data — it is now the most evictable.
      weakest = candidate;
    } else if (b < a) {
      weakest = candidate;
...
/**
 * Section-scoped weakest-live-pick finder (2026-08-19) — fixes a real,
 * serious pre-existing bug: the original findWeakestLivePick() (below)
 * compares ALL live tickers on raw momentum regardless of section. A calm,
 * stable Long stock is SUPPOSED to have low momentum — compared against
 * Short/Penny movers on that same yardstick, a genuinely great Long pick
 * would always look "weakest" and get evicted first, purely for doing
 * exactly what it's supposed to do. This version compares tickers ONLY
 * within their own section, using each stock's own sectionRank (the
 * section-appropriate metric — longTermRank-derived for Long, etc.),
 * not cross-section momentum. Same null-handling fix as the original
 * (missing/uncomputable rank = most evictable, never protected).
 */
async function findWeakestInSection(section, excludeTickers = []) {
  const { assessBoardPlacement } = require("./rankingStability");
  const exclude = new Set(
    (excludeTickers || []).map((t) => String(t).toUpperCase())
  );
  const picks = (await listLiveBoardPicks()).filter(
  ...
  for (const pick of picks) {
    ...
    try {
      ...
      if (entry?.data?.quote) {
        const placement = assessBoardPlacement(pick, entry.data);
        if (placement.boardSection === section) {
          rank = Number(placement.sectionRank);
        }
      }
    } catch {
      rank = null;
    }
    const candidate = { ...pick, sectionRank: Number.isFinite(rank) ? rank : null };
    if (!weakest) {
      weakest = candidate;
      continue;
    }
    const a = weakest.sectionRank;
    const b = candidate.sectionRank;
    if (a == null && b == null) {
      if (String(candidate.added_at) < String(weakest.added_at)) weakest = candidate;
    } else if (a == null) {
      // weakest already has no computable rank — stays most evictable.
    } else if (b == null) {
      weakest = candidate;
    } else if (b < a) {
      weakest = candidate;
...
async function countLiveInSection(section) {
  const row = await dbGet(
    `SELECT COUNT(*) AS n FROM board_picks
     WHERE status IN (${liveStatusSql()}) AND board_section = ?`,
    [...LIVE_BOARD_STATUSES, section]
  );
  return Number(row?.n || 0);
}

/**
 * Per-category capacity enforcement (2026-08-19) — evicts the weakest pick
 * WITHIN the target section only, comparing apples to apples, never
 * cross-section. Reuses SECTION_MAX_SIZE (default 15/category).
 */
async function ensureSectionCapacity(section, neededSlots = 1, { protect = [], max = SECTION_MAX_SIZE } = {}) {
  const archived = [];
  let live = await countLiveInSection(section);
  const need = Math.max(0, Number(neededSlots) || 0);
  const cap = Math.max(1, Number(max) || SECTION_MAX_SIZE);
  while (live + need > cap) {
    const weak = await findWeakestInSection(section, protect);
    if (!weak) break;
    archived.push(await archivePick(weak.ticker, { reason: "section_cap" }));
    live = await countLiveInSection(section);
  }
  return { section, liveCount: live, archived, max: cap };
}
...
async function purgeTickerData(tickers) {
  ...
  const tables = [
    ...
    "ai_summaries",
    "ai_deeper_looks",
    // Added 2026-08-17 — had a ticker column and genuinely per-ticker
    // operational data (logged recommendation outcomes), but was never
    // included here, meaning rows for permanently-purged tickers
    // accumulated forever as orphans. price_history_log is intentionally
    // NOT included — it's explicitly designed as a permanent historical
    // dataset that should outlive a ticker's board tenure, not per-ticker
    // operational state.
    "recommendation_log",
  ];
  let removed = 0;
...
/**
 * Shared delete-and-purge sequence — was duplicated verbatim in both
 * purgeOldArchived() and trimArchiveToCap() (identical DELETE +
 * purgeTickerData call, differing only in how each selects its ticker
 * list). Consolidated 2026-08-17 so there's one place to fix if this
 * sequence ever needs to change, instead of two that could drift apart.
 */
async function deleteArchivedTickers(tickers) {
  if (!tickers.length) return { removed: [], dataRowsDeleted: 0 };
  const placeholders = tickers.map(() => "?").join(", ");
  await dbRun(
    `DELETE FROM board_picks
  ...
  return { removed: tickers, dataRowsDeleted };
}
...
async function purgeOldArchived({
  ...
  const { dataRowsDeleted } = await deleteArchivedTickers(tickers);
  console.log(
...
/**
 * Monthly archive safeguard: retention-day purge then enforce ARCHIVE_MAX_SIZE.
 * (Comment previously said a stale hardcoded "40-cap" — the real cap was
 * raised to 200 on 2026-08-16; the code always used the real ARCHIVE_MAX_SIZE
 * variable correctly, this was a documentation-only fix.)
 */
...
module.exports = {
  ...
  findWeakestLivePick,
  findWeakestInSection,
  countLiveInSection,
  ensureSectionCapacity,
  archivePick,
  promotePick,
  upsertCandidateFromMover,
  upsertCandidateFromUniverse,
  upsertCandidateFromFactor,
