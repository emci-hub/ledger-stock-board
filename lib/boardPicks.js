...
async function promotePick(
  ticker,
  { status = "watch", sector = null, source = "discovery", name = null } = {}
) {
  const symbol = String(ticker).toUpperCase();
  const now = new Date().toISOString();
  const existing = await getPick(symbol);
  ...
  if (existing) {
    ...
    await dbRun(
      `INSERT INTO board_picks
        (ticker, status, added_at, sector, archived_at, source, miss_streak, tracked_since, name)
       VALUES (?, ?, ?, ?, NULL, ?, 0, ?, ?)`,
      [symbol, status, now, sector ?? null, source, trackedSince, name ?? null]
    );
  }
  console.log(
