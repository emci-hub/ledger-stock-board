...
function deriveDataCaveats(report, options = {}) {
  ...
  if (hasPrice && sma200 == null) {
    if (isIpoWithinLastYear(report.ipoDate)) {
      active.push({
        ...DATA_CAVEATS.limited_history,
        label: "Newly public",
        tooltip:
          "Newly public, limited trading history — this stock IPO'd within the last year, so longer averages aren't reliable yet.",
      });
    } else {
