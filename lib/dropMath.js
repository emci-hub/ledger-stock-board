...
const NEW_LOW_WINDOW_BARS = 20;
const SMA20_WINDOW_BARS = 20;

/**
 * Gate 6 ("stopped getting worse") inputs: how many of the most recent
 * sessions did NOT make a new 20-day low, and how many of the most recent
 * closes sit above the trailing 20-day MA. Both count back from the latest
 * bar and stop at the first session that breaks the streak. Returns nulls
 * when there isn't enough history to evaluate even the latest bar.
 */
function computeStoppedWorsening(closes) {
  const bars = sortedByDate(closes);
  if (bars.length < NEW_LOW_WINDOW_BARS + 1) {
    return { sessionsSinceNewLow: null, closesAboveSma20Count: null };
  }

  let sessionsSinceNewLow = 0;
  for (let i = bars.length - 1; i >= NEW_LOW_WINDOW_BARS; i--) {
    const priorWindow = bars.slice(i - NEW_LOW_WINDOW_BARS, i);
    const priorLow = Math.min(...priorWindow.map((b) => b.close));
    if (bars[i].close < priorLow) break;
    sessionsSinceNewLow += 1;
  }

  let closesAboveSma20Count = 0;
  for (let i = bars.length - 1; i >= SMA20_WINDOW_BARS - 1; i--) {
    const window = bars.slice(i - SMA20_WINDOW_BARS + 1, i + 1);
    if (window.length < SMA20_WINDOW_BARS) break;
    const sma20 = window.reduce((sum, b) => sum + b.close, 0) / SMA20_WINDOW_BARS;
    if (bars[i].close <= sma20) break;
    closesAboveSma20Count += 1;
  }

  return { sessionsSinceNewLow, closesAboveSma20Count };
}
