...
function resolveBoardSection(signals) {
  ...
  // 3) Long — established + stable. An extreme single-day move is not
  //    "calm" and always excludes a stock from Long that day — no override
  //    (the Dip Watch AI-confirmed-overreaction exception was retired along
  //    with the AI long-term rank; see lib/longTermVerdict.js for the real,
  //    deterministic ignore/watch/add/avoid verdict a Long-classified stock
  //    gets instead).
  ...
  const calmDay = !signals.extremeMove;
  if (
    historyOk &&
    volOk &&
    capOk &&
...
/** Stable placeholder — Long no longer has an AI-derived conviction score
 * to rank on (retired with Dip Watch/RANKING_RUBRIC.longTerm). The real
 * signal for a Long-classified stock is its ignore/watch/add/avoid verdict
 * (lib/longTermVerdict.js, stored on board_picks.long_term_verdict), not a
 * 0-100 number — display code should sort/show from that instead. */
const LONG_SECTION_RANK_PLACEHOLDER = 50;

/**
 * Category-local ranks — never cross-wire these between sections.
 */
function computeSectionRanks(report, signals) {
  const shortRaw =
  ...
  return {
    long: LONG_SECTION_RANK_PLACEHOLDER,
    short: shortRank,
...
function assessBoardPlacement(pick, report) {
  ...
  const ranks = computeSectionRanks(report, signals);
  ...
  return {
    ...
    signals,
    warnings,
    // Stable placeholders — Long no longer has an AI-derived rank to report
    // "raw" vs. "adjusted" for (see LONG_SECTION_RANK_PLACEHOLDER above).
    longTermRankRaw: ranks.long,
    longTermRankAdjusted: ranks.long,
