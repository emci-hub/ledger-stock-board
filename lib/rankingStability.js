...
/** Real, standard technical indicator (RSI, Wilder 1978) — below this is the
 * traditional "oversold" reading. Requiring RSI confirmation alongside the
 * price drop means both a real price shock AND real momentum exhaustion
 * must agree before the trigger fires — fewer, more meaningful triggers,
 * per explicit decision (2026-08-17), not just reacting to any drop. */
const DIP_WATCH_RSI_OVERSOLD_THRESHOLD = Math.max(
  1,
  Number.parseFloat(process.env.DIP_WATCH_RSI_OVERSOLD_THRESHOLD || "30") || 30
);
...
function computeDipWatchTrigger(report = {}, priorBoardSection = null, opts = {}) {
  ...
  const rsiThreshold = Number.isFinite(opts.rsiOversoldThreshold)
    ? opts.rsiOversoldThreshold
    : DIP_WATCH_RSI_OVERSOLD_THRESHOLD;

  if (priorBoardSection !== BOARD_SECTIONS.LONG) {
    return { triggered: false, windowDays, windowPct: null, rsi: null };
  }
  ...
  const priceDropped = windowPct <= thresholdPct;

  // RSI confirmation — both signals must agree, not just the price drop
  // alone. Missing RSI data (rare, but possible on thin-history tickers)
  // means we can't confirm momentum exhaustion — fails closed (does NOT
  // trigger), consistent with the same "missing data must not silently
  // pass" principle already applied to Long's other gates.
  const rsi = Number(
    report.indicators?.short?.rsi ?? report.quote?.indicators?.short?.rsi
  );
  const rsiConfirmed = Number.isFinite(rsi) && rsi < rsiThreshold;

  return {
    triggered: priceDropped && rsiConfirmed,
    windowDays,
...
const DIP_WATCH_AI_SHORTLIST_SIZE = Math.max(
  1,
  Number.parseInt(process.env.DIP_WATCH_AI_SHORTLIST_SIZE || "20", 10) || 20
);

/**
 * Deterministic dip-severity ranking — zero AI cost, pure numbers. When many
 * Long stocks trigger Dip Watch at once (e.g. a market-wide selloff), this
 * ranks them by real severity (bigger drop + lower RSI = more severe) so
 * only the most severe candidates get sent to Gemini for deep reasoning —
 * explicit decision (2026-08-17): AI should never be asked to triage a
 * batch of numbers, that's what this function is for. AI is reserved for
 * the judgment call ("is this a real problem or an overreaction") on the
 * already-narrowed shortlist.
 */
function rankDipWatchCandidatesBySeverity(candidates) {
  return (candidates || [])
    .filter((c) => Number.isFinite(c?.windowPct) && Number.isFinite(c?.rsi))
    .map((c) => {
      const dropSeverity = Math.max(0, -c.windowPct);
      const rsiSeverity = Math.max(
        0,
        DIP_WATCH_RSI_OVERSOLD_THRESHOLD - c.rsi
      );
      return { ...c, severity: dropSeverity + rsiSeverity };
    })
    .sort((a, b) => b.severity - a.severity);
}

/** Top N most severe REAL triggered candidates. Pure/testable, no AI, no side effects. */
function selectTopDipWatchCandidates(candidates, limit) {
  const n = Math.max(1, Number(limit) || DIP_WATCH_AI_SHORTLIST_SIZE);
  return rankDipWatchCandidatesBySeverity(candidates).slice(0, n);
}

const DIP_WATCH_SHORTLIST_SETTING_KEY = "dip_watch_shortlist";

/**
 * Once-per-calendar-day pre-scan: for every currently-Long live ticker,
 * compute its dip trigger using ALREADY-CACHED data (no new API calls),
 * rank all real triggers by severity, and store only the top N ticker
 * symbols. Called once at the start of the daily refresh (refreshBoard),
 * BEFORE the main per-ticker loop — the loop then only sends "triggered"
 * to Gemini for tickers on this shortlist, even if their fresh same-day
 * data would also independently qualify. This is what keeps a genuine
 * market-wide selloff (many stocks dipping at once) bounded to a fixed,
 * small number of deep-reasoning AI calls instead of scaling with however
 * many stocks happen to be dipping that day.
 */
async function computeDailyDipWatchShortlist(tickers, options = {}) {
  const { getSetting, setSetting } = require("../services/usage");
  const { getPick } = require("./boardPicks");
  const { getStockCacheEntry } = require("../services/cache");
  const limit = Number(options.limit) || DIP_WATCH_AI_SHORTLIST_SIZE;

  const candidates = [];
  for (const ticker of tickers || []) {
    let pick;
    try {
      pick = await getPick(ticker);
    } catch {
      continue;
    }
    if (!pick || pick.board_section !== BOARD_SECTIONS.LONG) continue;

    let entry;
    try {
      entry = await getStockCacheEntry(ticker);
    } catch {
      continue;
    }
    const report = entry?.data?.quote
      ? { priceHistory: entry.data.quote.priceHistory, indicators: entry.data.quote.indicators }
      : null;
    if (!report) continue;

    const trigger = computeDipWatchTrigger(report, BOARD_SECTIONS.LONG);
    if (trigger.triggered) {
      candidates.push({ ticker, windowPct: trigger.windowPct, rsi: trigger.rsi });
    }
  }

  const shortlist = selectTopDipWatchCandidates(candidates, limit);
  const tickerList = shortlist.map((c) => c.ticker);
  const today = new Date().toISOString().slice(0, 10);

  try {
    await setSetting(
      DIP_WATCH_SHORTLIST_SETTING_KEY,
      JSON.stringify({ date: today, tickers: tickerList })
    );
  } catch (err) {
    console.warn("[dipWatch] failed to store shortlist:", err.message);
  }

  console.log(
    `[dipWatch] Daily shortlist: ${candidates.length} real triggers found, top ${tickerList.length} selected for AI review: ${tickerList.join(", ") || "(none)"}`
  );
  return { consideredCount: candidates.length, shortlist: tickerList };
}

/**
 * Reads today's shortlist. If it hasn't been computed yet today (e.g. this
 * ran before computeDailyDipWatchShortlist did, or that step failed),
 * returns null — callers should treat null as "fail open" (don't block a
 * legitimate trigger just because the pre-scan hasn't run), NOT as "empty
 * shortlist" (which would incorrectly block everything).
 */
async function getTodaysDipWatchShortlist() {
  const { getSetting } = require("../services/usage");
  try {
    const raw = await getSetting(DIP_WATCH_SHORTLIST_SETTING_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    const today = new Date().toISOString().slice(0, 10);
    if (parsed.date !== today) return null; // stale from a prior day
...
module.exports = {
  ...
  DIP_WATCH_THRESHOLD_PCT,
  DIP_WATCH_RSI_OVERSOLD_THRESHOLD,
  DIP_WATCH_AI_SHORTLIST_SIZE,
  rankDipWatchCandidatesBySeverity,
  selectTopDipWatchCandidates,
  computeDailyDipWatchShortlist,
  getTodaysDipWatchShortlist,
