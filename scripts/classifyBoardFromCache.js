...
async function main() {
  ...
  const bySection = { long: [], short: [], penny: [] };
  ...
  for (const pick of picks) {
    ...
    counts[row.boardSection] += 1;
    bySection[row.boardSection].push(row.ticker);

    const s = row.placement.signals;
    if (s.insufficientToClassify) {
    ...
    JSON.stringify(
      {
        inventory: {
          ...
          classifiedWithCache: Object.values(counts).reduce((a, b) => a + b, 0),
          skippedNoCache,
        },
