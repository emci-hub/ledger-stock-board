...
/**
 * TEMPORARY — live dry-run of the Gemini analysis pipeline against a single
 * ticker, to confirm the trimmed GEMINI_RESPONSE_SCHEMA (longTermRank/
 * dipWatch removed) still returns valid, parseable JSON. Real Gemini call,
 * synthetic quote/fundamentals input (isolates the schema/prompt question —
 * no other providers touched). Same password gate as /api/dev/status.
 * Remove this route once confirmed — it is not meant to stay in the app.
 */
app.get("/api/dev/analyze-live-test", async (req, res) => {
  if (!devAuthOk(req)) return rejectDevUnauthorized(res);
  try {
    const { analyzeStock } = require("./services/analyze");
    ...
    const quoteData = {
      ...
      indicators: {
        short: { sma20: 198, sma50: 195, rsi: 55, macd: {}, bollinger: {} },
        long: {
          sma20: 198,
          sma50: 195,
          sma200: 180,
      ...
      priceHistory: Array.from({ length: 260 }, (_, i) => 150 + i * 0.2),
      historyBars: 260,
    };
    ...
    const result = await analyzeStock(ticker, quoteData, fundamentalsData, [], {
      provider: "gemini",
    });
    const looksLikeRealAnalysis =
      result?.short?.summary &&
      !String(result.short.summary).toLowerCase().includes("wasn't available");

    return res.json({
      ticker,
      generatedAt: new Date().toISOString(),
      looksLikeRealAnalysis,
      result,
    });
  } catch (err) {
    console.error("[GET /api/dev/analyze-live-test]", err.message);
